...
export const NODES = {
  ...
  welcome: {
    ...
    options: [
      ...
      },
      {
        ...
        sub: "拿着不舒服，又不知道该不该走",
        next: "b_import",
        ack: "拿不准的时候最难受。我们把它拆开看看——多半是当初的理由现在说不清了。",
      },
      {
        title: "还没想法，随便看看",
        sub: "想找点值得研究的东西",
        next: "c_theme",
        ack: "行，那我们从你感兴趣的方向开始找。",
      },
      {
        title: "先看别人怎么做的",
        ...
        sample: true,
        ack: "好，我用 NVDA 走一遍完整流程，你看看这个方法适不适合你。",
      },
    ],
    story: {
      title: "Welcome：先自我介绍，再问问题",
      why: "原来第一句是「你想把哪笔交易想清楚」——正确但冷。现在先说<strong>「我是你的投研助理」</strong>，一句话交代身份就直接进入正题。<strong>刻意不主动声明「不连券商、不会下单」</strong>——用户没提这个顾虑之前先解释，反而是在暗示这里有风险。选项也从功能命名（分析/复盘/探索）改成<strong>用户会对自己说的话</strong>（「我已经买了，但心里没底」）。",
      tags: ["TTFV < 90s", "不主动辩解", "用用户的话写选项"],
    },
  },
  ...
  a_direction: {
    ...
    options: [
      ...
      },
      {
        ...
        next: "a_horizon",
        set: { direction: "观望" },
        ack: "那关键就是「等什么」。我们把这个信号定义到能验证的程度，它出现时我告诉你。",
      },
      {
        title: "真说不准",
        sub: "所以才想找人捋一捋",
    ...
    options: [
      {
        ...
        next: "a_thinking",
        set: { why: "财报催化剂" },
        ack: "财报是个明确的验证点。不过要注意：市场交易的是预期差，不是好坏本身。我把这次的一致预期拉给你，你看看你的判断和它差在哪。",
      },
      {
        title: "跌了一波，我觉得跌过头了",
        sub: "想抄个底",
        next: "a_thinking",
        set: { why: "超跌反弹" },
        ack: "「跌过头」需要一个参照才成立——相对什么？我把它的估值分位拉出来，你看看是真便宜，还是只是比之前便宜。",
      },
      {
        title: "看到了什么消息",
        sub: "新产品、政策、并购之类的",
        next: "a_thinking",
        set: { why: "事件催化剂" },
        ack: "消息是个起点。关键是它传开了没有——如果已经人尽皆知，价格里通常就没剩多少了。我查一下扩散程度。",
      },
      {
        title: "图形看着要动了",
        sub: "突破、站上均线之类的",
        next: "a_thinking",
        set: { why: "技术形态" },
        ack: "图形能告诉你什么时候，但告诉不了为什么。我把基本面对一下——两边指向一致的时候，这个判断才站得住。",
      },
      {
        title: "说不上来，就是有感觉",
        sub: "可能最近老看到它",
        ...
        next: "a_thinking",
        set: { why: "待 AI 归因" },
        ack: "直觉一般有来处，只是一时说不清。我回溯一下最近的信息，看能不能帮你把它还原成具体的东西——还原不出来，那这个判断可能就还不够扎实。",
      },
    ],
    story: {
      title: "命题 4/4：为什么是现在",
      why: "加了一句<strong>「这只票一直都在，你偏偏这几天想起它——总有个由头」</strong>，把抽象问题变成用户能回答的问题。每个 ack 都<strong>接住动机，同时补上一个它自己不包含的检验条件</strong>：说「财报快到了」，补「市场交易的是预期差」；说「跌过头了」，追问「相对什么」；说「图形要动了」，指出「图形说不了为什么」。<strong>不否定用户的出发点，但也不假装它已经完整。</strong>",
      tags: ["把抽象问题具象化", "接住动机+补检验条件", "不给空白编辑器"],
    },
  },
  ...
  a_goal: {
    ...
    options: [
      ...
      },
      {
        ...
        next: "a_constraint",
        set: { goalKind: "限定损失" },
        ack: "那我优先给你看最大亏损写死的结构——先知道最坏情况，再看能赚多少。",
      },
      {
        title: "已经有仓位了，想多赚点",
        sub: "在现有基础上加点收益",
      ...
      },
      {
        ...
        next: "a_constraint",
        set: { goalKind: "等待确认" },
        ack: "等可以，但得等一个具体的东西。我们把它定义到能验证的程度，到了我通知你。",
      },
    ],
    story: {
    ...
    options: [
      {
        ...
        next: "a_express",
        set: { budget: "最大损失 ≤ $500" },
        ack: "好，$500。仓位小的时候人比较拿得住，判断也不容易被浮盈浮亏带跑。",
      },
      {
        title: "$2,000 上下",
        sub: "正常仓位",
      ...
      },
      {
        ...
        ack: "$5,000。这个量级下，我把最大亏损标得更醒目一些——你确认一下那个数字是不是真的能接受。",
      },
      {
        title: "没想好",
        sub: "帮我定一个",
  ...
  a_express: {
    ...
    story: {
      title: "交易表达：情景损益必须同屏",
      why: "每个表达都展示<strong>所需资金、最大损失、盈亏平衡、牛/基准/熊损益</strong>，并直接标注是否满足用户刚才设的约束（绿色 ✓ / 红色 ✗）。<strong>首次 onboarding 不出现下单按钮</strong>——这一屏的目的是让用户看懂自己的判断值多少钱、能亏多少，而不是促成交易。",
      tags: ["情景损益同屏", "无下单按钮", "约束适配可见"],
    },
  },
  ...
  b_import: {
    ...
    story: {
      title: "入口 B：券商连接不是门槛",
      why: "issue 里最重要的一条策略：<strong>连券商是价值展示后的信任动作，不是 activation 门槛</strong>。所以这里截图导入是推荐项，只读连接排在最后。<strong>但这件事只体现在选项排序上，不写进文案</strong>——主动强调「不连也能走完」反而会让用户觉得这里有坑。连接失败绝不阻断首次体验。",
      tags: ["截图/手动 fallback", "用排序表达优先级", "连接失败不阻断"],
    },
  },
    ...
    options: [
      {
        ...
        ack: "那是个长周期的命题。我们对一下这一年它兑现到哪了——兑现节奏和你当初的预期差多少，比涨跌更能说明问题。",
      },
      {
        title: "当时跌了不少，想抄底",
        sub: "觉得便宜",
        ...
        ack: "抄底的前提是当时确实低估了。我拉一下估值分位——看看是判断对了，还是只是买在了半山腰。",
      },
      {
        title: "别人推荐的",
        sub: "朋友、博主，或者刷到的",
        ...
        ack: "想法可以来自别人，但仓位是你的——涨跌也是你承担。今天我们把依据补上，之后拿不拿得住，你自己说了算。",
      },
      {
        title: "……真的想不起来了",
        sub: "太久了",
    ...
    options: [
      {
        ...
        sub: "不想全走，但也扛不住这么多",
        next: "a_rule",
        set: { structure: "减仓至 1/3", goalKind: "限定损失" },
        ack: "好。减到多少是你能睡着的量？这个数字定下来，剩下的仓位就是你真正愿意承担的部分。",
      },
      {
        title: "全出了吧",
        sub: "理由变了，那就重新开始",
        next: "a_rule",
        set: { structure: "清仓退出", goalKind: "限定损失" },
        ack: "收到。走之前把「当初为什么买、现在什么变了」记下来——这是这笔交易唯一能留下的东西。",
      },
      {
        title: "再给它一次机会",
        sub: "但我想设条底线",
        next: "a_rule",
        set: { structure: "维持持仓 + 硬止损", goalKind: "等待确认" },
        ack: "可以。那「再一次机会」具体指什么——等哪个数据、等到什么时候？说得出来，它就是计划；说不出来，它就是拖。",
      },
      {
        title: "我还想再想想",
        sub: "先不做决定",
        kind: "relief",
        next: "a_rule",
        set: { structure: "暂不调整 · 待复盘", goalKind: "等待确认" },
        ack: "完全可以，这种事本来就不该当场拍板。我们约个时间，到时候我把最新情况整理好给你。",
      },
    ],
    story: {
      title: "复盘裁决：不评价选择，只追问条件",
      why: "四个选项没有对错之分，开场就明说<strong>「都是合理的选择，没有标准答案」</strong>——降低选择焦虑。但<strong>不评价用户选得好不好</strong>：选「全出」不夸果断，而是提醒把「什么变了」记下来；选「再给一次机会」不夸有底线，而是追问<strong>「那『机会』具体指什么」</strong>——说得出来是计划，说不出来是拖。<strong>让用户自己检验逻辑，比替他下判断有用。</strong>",
  ...
  c_theme: {
    ...
    options: [
      ...
      },
      {
        ...
        next: "c_pick",
        set: { theme: "生活消费倒推" },
        ack: "一手体感常常比研报早。不过好产品和好股票是两件事——中间还隔着价格，我们等下会对一下。",
      },
    ],
    story: {
    ...
    options: [
      {
        ...
        ack: "这条盯的是你买入的理由本身，比盯价格更接近问题的核心。数据出来我第一时间对给你。",
      },
      {
        title: "跌破 $158",
        sub: "跌到这我就认",
        ...
        ack: "价格条件的好处是没有解释空间，到了就是到了。到价我提醒你，届时再结合当时的情况判断。",
      },
      {
        title: "两个哪个先到算哪个",
        sub: "双保险",
        ...
        ack: "逻辑和价格都盯上了。两条里先到哪条都算数——这样不容易出现「跌了但我觉得逻辑还在」的自我说服。",
      },
      {
        title: "我真定不出来",
        sub: "你先给我个参考",
